...
#if defined(NDEBUG)
#error "MasternodeToken cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uMAST) are considered zero fee (for relaying and mining)
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight > 1 && nHeight <= 21)
        nSubsidy = 59154 * COIN;
    else if (nHeight > 21 && nHeight <= 525600)
        nSubsidy = 10 * COIN;
...
{
    RenameThread("MasternodeToken-scriptch");
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // MAST: We use certain sporks during IBD, so check to see if they are
